...
pub enum NetDescriptorParseError {
  ...
  #[error("invalid value '{0}': must start with either tcp:// or udp://")]
  InvalidListenSpecifier(String),
  #[error("{0}")]
...
impl NetListenDescriptor {
  pub fn parse(specifier: &str) -> Result<Self, NetDescriptorParseError> {
    let protocol = if specifier.starts_with("tcp://") {
      ...
      return Err(NetDescriptorParseError::InvalidListenSpecifier(specifier.to_string()));
    };

    Ok(NetListenDescriptor(NetDescriptor::parse(&specifier[6..])?, protocol))
  }

  pub fn from_url(url: &Url, protocol: Protocol) -> Result<Self, NetDescriptorFromUrlParseError> {
